// scripts/buildBgNebulaSvgAtlasMulti.js
import fs from "fs";
import path from "path";
import { optimize } from "svgo";

const inDir = path.join(process.cwd(), "output", "bgNebula");
...
const outContractPath = path.join(outContractsDir, "BgNebulaAtlas.sol");
...
function ensureHasNebulaGroup(fragment, fileName) {
  if (!/id=["']bgNebula["']/.test(fragment)) {
    throw new Error(
      `Missing id="bgNebula" group in ${fileName}. Make sure it contains <g id="bgNebula">...</g>.`
    );
  }
}
    ...
    plugins: [
      {
        ...
        params: {
          overrides: {
            cleanupIds: false, // keep bgNebula + gradient ids
          },
        },
      },
...
function main() {
  console.log("Building BgNebulaAtlas from:", inDir);
  ...
  for (const fileName of files) {
    ...
    ensureHasNebulaGroup(rawInner, fileName);

    const minInner = minifyFragment(rawInner);
    ensureHasNebulaGroup(minInner, fileName);
  ...
  fragments.forEach((f, i) => {
    ...
    out += `        nebulaPointers.push(SSTORE2.write(hex"${hexData}"));\n`;
  });

  out += solidityFooter();

  fs.writeFileSync(outContractPath, out, "utf8");
  console.log("Wrote BgNebulaAtlas.sol →", outContractPath);
